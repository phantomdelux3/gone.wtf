...
export const BuyForm: FC = () => {
  ...
  // --- 1. pull sale account whenever wallet changes ---
  useEffect(() => {
    if (!wallet.publicKey) { setSale(null); return; }
    (async () => {
      try {
        if (!wallet.publicKey) {
          return;
  ...
  if (!wallet.connected) return <p>Please connect a wallet.</p>;
  ...
  return (
    <form id='buytoken' onSubmit={submit} className="max-w-md space-y-4 z-40">
      ...
      <label className="block">
        ...
        <input
          type="number"
          min={sale.min.div(new BN(LAMPORTS_PER_SOL)).toNumber()}
          max={sale.max.div(new BN(LAMPORTS_PER_SOL)).toNumber()}
          step="0.01"
          value={solInput}
